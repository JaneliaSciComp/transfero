...
def test_transfero(do_transfer_data_from_rigs=True, do_run_analysis=True) :
    ...
    rig_host_name = '10.103.30.38' 
    rig_user_name = 'OTOPALIKLAB-WW6+labadmin' 
    rig_data_folder_path = '/cygdrive/d/Data/scicompsoft' 
    rig_user_folder_path = os.path.join(rig_data_folder_path, 'taylora')
    transfero_destination_folder_path = os.path.join(root_example_experiments_folder_path, 'test-transfero-destination-folder') 
    transfero_destination_user_folder_path = os.path.join(transfero_destination_folder_path, 'taylora')
    ...
    per_lab_configuration['user_name_for_configuration_purposes'] = 'otopalikrobot'
    ...
    if do_transfer_data_from_rigs :
        ...
        command_line = 'scp -o StrictHostKeyChecking=accept-new -B -r %s/* %s@%s:%s' % (read_only_example_experiments_folder_path, rig_user_name, rig_host_name, rig_user_folder_path)
    ...
    local_verify(read_only_example_experiments_folder_path, transfero_destination_user_folder_path) 

    # Check the return values from transfero()
    if len(relative_path_from_synched_experiment_folder_index)==1 and len(job_status_from_experiment_index)==0:
        # all is well
        pass
    else :
    ...
    # Check that the expected files are present on prfs
